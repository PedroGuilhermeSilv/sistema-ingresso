...
export default async function SpotsLayoutPage({
    ...
    const uniqueRows = rowLetters.filter(
        (rowLetter: string, index: number) => rowLetters.indexOf(rowLetter) === index
    );

    const spotGroupedByRow = uniqueRows.map((rowLetter) => {
        return {
            rowLetter,
            spots: [
                ...spots.filter((spot) => spot.name[0] === rowLetter).sort((a, b) => {
        ...
        <main className="mt-10">
            ...
            <div className="mt-6 flex flex-wrap justify-between">
                <div className=" mb-4 flex w-full max-w-[650px] flex-col gap-y-8 rounded-2xl bg-secondary p-6">
                    ...
                    <div className="md:w-full md:justify-normal">
                        {spotGroupedByRow.map((row) => {
                            return (
                                <div key={row.rowLetter} className="flex flex-row gap-3 items-center mb-3">
                                    <div className="w-4"> {row.rowLetter}</div>
                ...
                <div className="flex w-full max-w-[478px] flex-col gap-y-6 rounded-2xl bg-secondary px-4 py-6">
                    ...
                    <p>
                        Inteira: {formattedPrice(event.price)} <br />
                        Meia-entrada: {formattedPrice(event.price / 2)}
                    </p>
